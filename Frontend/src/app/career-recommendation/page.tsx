...
const gridButtons = [
  ...
  { id: 'salary', title: 'Salary & Job Roles', icon: DollarSign, color: 'hover:border-yellow-500/50 hover:shadow-[0_0_15px_rgba(234,179,8,0.2)] text-yellow-300' },
]
  ...
  useEffect(() => {
    ...
    async function loadPersonalization() {
      try {
        ...
        console.warn('Personalization unavailable:', e)
      } finally {
        setLoadingPersonalization(false)
      }
    }
              ...
              {gridButtons.map((btn) => {
                ...
                return (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    className={`flex items-center justify-between p-3.5 px-4 rounded-2xl border transition-all text-left group bg-white/5 backdrop-blur-md cursor-pointer ${
                      isActive
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <BtnIcon className={`h-4.5 w-4.5 shrink-0 ${isActive ? 'text-indigo-400' : ''}`} />
                      <span className="text-xs font-semibold leading-snug">{btn.title}</span>
                    </div>
